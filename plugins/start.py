...
# When users start the bot without a special link
...
async def start_command(client: Client, message: Message):
    ...
    text = message.text
    if len(text) > 7:  # Special link (Base64 Encoded)
        try:
        ...
        if len(argument) >= 2:
            ...
            # Handle file delivery after subscription check
            if len(argument) == 3:
                try:
            ...
            for msg in messages:
                caption = (CUSTOM_CAPTION.format(previouscaption="" if not msg.caption else msg.caption.html, filename=msg.document.file_name) if bool(CUSTOM_CAPTION) & bool(msg.document)
        ...
        # General /start message (No special link)
...
# Command for admin to broadcast messages
